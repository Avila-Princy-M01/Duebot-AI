...
This benchmark evaluates `backend/llm/reply_parser.py` across 50 ground-truth B2B buyer replies.
...
1. **Zero False-Positive Commitments (100% Precision)**:
   Across all classes, every high-confidence classification (`confidence >= 0.7`) is 100% accurate.
...
2. **High Automation Rate on Outbound Risk**:
   ...
   - `promise` (**75.0% Recall, 85.7% F1**): 3 out of 4 explicit promises extracted automatically.

3. **Deliberate Safety-First Abstention (32.0%)**:
   The remaining 25% of promises and 30% of disputes landing in `ambiguous` are borderline or
   code-mixed statements where confidence falls below 0.70. `engine/policy.py` intercepts these
   and routes them to `HUMAN_REVIEW` — eliminating money-adjacent automation errors while
   automating 88% of unambiguous B2B interactions.
"""
    return md

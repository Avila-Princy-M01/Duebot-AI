...
**3:00–4:00 — Audit + metrics.** Scroll the append-only log for one invoice. Then open the 3-Way Metrics evaluation: no-agent vs naive 7-day cadence vs DueBot on held-out generator splits (`python scripts/run_multi_seed_eval.py`). The headline is zero-defect safety + message efficiency verified across 10 random seeds: DueBot delivers 100% dispute protection (0.0 touches vs Naive sending 5.3 to 13.4 harassment touches), 61.5% fewer messages overall (46%–61% fewer across all contact budgets), and 0 false promises while recovering the full responsive cohort.
